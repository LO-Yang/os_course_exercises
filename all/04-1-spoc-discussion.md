...
答：
* 各存储器距离cpu从近到远依次为：寄存器、高速缓存、内存、外存；
* 访问时间从低到高依次为: 寄存器、高速缓存、内存、外存；
* 访问频率从低到高依次为: 外存、内存、高速缓存、寄存器；
* 存储容量从小到大依次为：寄存器、高速缓存、内存、外存；

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？
答：即让各层次的存储器都处于繁忙且繁忙程度相对平均的工作状态，使得不至于相互拖累。

3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
答：在超大数组排序时内存不够使用. 解决方案是归并排序，并将暂时归并好、不需要立刻使用的数据放入外存

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？
答：覆盖技术，即让不会同时执行的模块放入同一块内存。程序员需要划分模块，并确定模块的共享与否.

2. 什么是交换技术？覆盖与交换有什么不同？
答：交换技术是让暂时不运行的程序先放入外存.
不同在于，覆盖技术解决的是一个大内存进程的运行问题(分块)
交换解决的是多进程共占内存导致内存不够的问题(整体)

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？
答：现代计算机系统中，覆盖已经不用了，但交换仍有需要.
...
2. 如何提高程序执行时的局部性特征？
答：编写程序时要注意内存访问(访问数据或指令跳转)尽可能相近

3. 排序算法的局部性特征？
  * 参考：[九大排序算法再总结](http://blog.csdn.net/xiazdong/article/details/8462393)
答：归并排序、桶排序需要较大存储空间，局部性特征较差；而冒泡、快速、选择、插入的局部性特征优越一些。
 ...
 * 缺页中断处理可以将需要访问的页面从外存拉入内存，并将被替换的内存中的页面写入硬盘。
 ...
 * 保护位：表示允许页面访问方式
 ...
 3. 页式存储和虚拟页式存储的区别是什么？
 呃...我一直以为页式存储就是虚拟页式存储的意思啊。

### 8.6 缺页异常

1. 缺页异常的处理流程？
* A.在内存中有空闲物理页 面时,分配一物理页帧f, 转第E步;
