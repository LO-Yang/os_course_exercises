...
* 在中断响应时，访问中断门，则当前段的权限级别(CS为CPL, 其余为RPL)是否低于0特权级将引发不同行为（是否换栈）
* 在函数调用时，访问目标段，要求当前段的权限级别(CS为CPL, 其余为RPL)要不低于目标段的DPL.
...
* 当代码访问数据时，需要检查当前特权级是否不低于要访问的数据段的DPL.
* 当访问栈段寄存器的时候，需要RPL、CPL均与DPL相等.
* 发生中断或异常时，需要判断当门描述符的特权级是否高于当前特权级

2. 在什么情况下会出现特权级切换？
* 低特权级代码调用高特权级代码，以及中断、异常时，会出现特权级切换.

3. int指令在ring0和ring3的执行行为有什么不同？
* int指令在ring0时，不需要切换运行栈，而在ring3时需要.
* 在ring0时，不需要将SS和ESP压栈，而ring3需要.

4. 如何利用int和iret指令完成不同特权级的切换？
* 需要人工构造栈结构

5. TSS和Task Register的作用是什么？
* TSS保存了一个进程的寄存器值等现场信息
* Task Register用于在全局描述符表中选择TSS的段描述符
...
答：若一条指令有两个内存寻址操作数，则会有四次地址转换

2. 描述X86-32的MMU地址转换过程；
答：虚拟地址通过段机制转换成线性地址，之后通过页机制转为物理地址。

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。
* 修改cr0寄存器，cpu将从实模式转化为保护模式，同时将使能页机制；后一条长跳转指令，将改变eip寄存器.
* 段寄存器将指向全局描述符表的内部地址，而非表示访问地址的高位

2. 分析页表的建立过程；
答：在内存中开辟页表空间，并将页表的基址赋予cr3寄存器的高16位。
